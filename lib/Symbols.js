...
Symbols.getLastSymbolTerm = function(node, symbolName) {
  ...
  // with `symbolName`
  else if (Node.Type.isOperator(node, '+') || Node.Type.isOperator(node, '-') || Node.Type.isOperator(node, '*') || Node.Type.isOperator(node, '/')) {
    for (let i = node.args.length - 1; i >= 0 ; i--) {
      const child = node.args[i];
      if (Node.Type.isOperator(child, '+') || Node.Type.isOperator(child, '-') || Node.Type.isOperator(child, '*') || Node.Type.isOperator(child, '/')) {
